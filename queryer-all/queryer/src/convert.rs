        ...
        match sql {
            Statement::Query(q) => {
                ...
                for expr in orders {
                    ...
                    // 将 sqlparser::ast::OrderBy 转换为你的 OrderByExpr 类型
                    // let order_by_expr = OrderByExpr {
                    //     expr: expr.expr.clone().try_into()?,
                    //     asc: expr.asc.unwrap_or(true),
                    //     nulls_first: expr.nulls_first,
                    // };
                    // let order = Order(expr);
                    // let order_by_expr: OrderByExpr = order.try_into()?;
...
impl TryFrom<Expression> for Expr {
    ...
    fn try_from(expr: Expression) -> Result<Self, Self::Error> {
        match *expr.0 {
            ...
            // SqlExpr::Identifier(id) => Ok(Self::Column(Arc::new(id.value.into()))),
    ...
    fn try_from(p: Projection<'a>) -> Result<Self, Self::Error> {
        match p.0 {
            ...
            SelectItem::ExprWithAlias {
                ...
                // Arc::new(alias.value.as_str()),
                Arc::new(Expr::Column(Arc::from(id.value.as_str()))),
...
impl<'a> TryFrom<Order<'a>> for (String, bool) {
    ...
    fn try_from(o: Order) -> Result<Self, Self::Error> {
        // 获取第一个 OrderByExpr（如果存在）
        let first_expr =
            o.0.exprs
                .first()
                .ok_or_else(|| anyhow::anyhow!("OrderBy is empty"))?;

        // 获取排序表达式
        let expr = &first_expr.expr;

        // 尝试将表达式转换为字符串
        let field = match expr {
            sqlparser::ast::Expr::Identifier(ident) => ident.value.clone(),
            _ => return Err(anyhow::anyhow!("Unsupported expression type for ordering")),
        };

        // 确定排序方向
        let asc = !matches!(first_expr.asc, Some(false));
